...
control 'apache-06' do
  ...
  describe apache_conf do
    its('User') { should eq [httpd.user] }
    its('Group') { should eq [httpd.user] }
  end
end
...
control 'apache-07' do
  ...
  describe file(File.join(httpd.conf_dir, '/conf-enabled/security.conf')) do
...
control 'apache-08' do
  ...
  module_path = File.join(httpd.conf_dir, '/mods-enabled/')
...
control 'apache-11' do
  ...
  describe file(File.join(httpd.conf_dir, '/mods-enabled/alias.conf')) do
...
control 'apache-13' do
  ...
  describe file(File.join(httpd.conf_dir, '/mods-enabled/ssl.conf')) do
    its('content') { should match(/^\s*?SSLHonorCipherOrder\s+?On/i) }
  end

  sites_enabled_path = File.join(httpd.conf_dir, '/sites-enabled/')
